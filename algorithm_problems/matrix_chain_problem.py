...
# For example if A is a 10x30 matrix, B is a 30x5 matrix, and C is a 5x60, 
# then computing ABC as A(BC) costs 27000 operations, because:
...
# (NB: To be able to multiply a matrix A of size n x m by a matrix B of size m x p, 
# the number of columns of A must be equal to the number of rows of B, 
# and the cost of the multiplication is n x m x p, and the result of the product is a matrix of size n x p)

# In the Matrix chain multiplication problem, we are given dimensions of n matrices, 
# and we are asked to calculate the minimum number of operations to calculate the product M0*M1*M2*...
...
# explanation: The optimal way of putting parentheses to compute ABCDE is (A(BC))(DE), 
# we calculate BC, then we multiply A by the result, then we calculate DE, 
# then we multiply the result of A(BC) by the result of DE
# (NB: A is matrices[0], B is matrices[1], C is matrices[2], 
# D is matrices[3], and E is matrices[4])
